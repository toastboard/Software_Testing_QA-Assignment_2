...
def retirement_savings_goal():
    ...
    if form.validate_on_submit():
        ...
        ret_obj = RetirementCalculator(age, salary, percent_savings,
                                       savings_goal)
        years_to_goal = ret_obj.calculate_savings_age()
        is_reachable = ret_obj.is_goal_reachable()
        age_at_goal = int(age) + int(years_to_goal)
        dict_to_retirement_page = {'age': age, 'salary': salary,
                                   'years_to_goal': f'{years_to_goal:,}',
                                   'percent_savings': percent_savings * 100,
                                   'savings_goal': savings_goal,
                                   'is_reachable': is_reachable,
                                   'age_at_goal': age_at_goal}
...
def body_mass_index():
    ...
    if form.validate_on_submit():
        ...
        if bmi_obj.get_bmi_category() == BodyMassIndexCategory.UNDERWEIGHT:
            ...
            bmi_category = "Obese"
        dict_to_bmi_page = {'weight': weight, 'height': height,
                            'bmi_value': bmi_value,
